		...
		if (dd < 0) {
			...
			if (files_too) {
				...
				while (fioDread(dd, &buf) > 0) {
					...
					if(filter)
						cond = FIO_SO_ISREG(buf.stat.mode) && ((strstr(strupr(filename), ".SMS") != NULL) || (strstr(strupr(filename), ".GG") != NULL));
					else
...
int listcdvd(const char *path, entries *FileEntry) {
	...
	CDVD_FlushCache();
	if(filter)
		n = CDVD_GetDir(dir, ".SMS .GG", CDVD_GET_FILES_ONLY, TocEntryList, FILEENTRY_SIZE, dir);
	else
...
char* Browser(int files_too, int menu_id, int filtered) {
	...
	oldselect = -1;
	//entries *FileEntry = malloc(sizeof(entries)*FILEENTRY_SIZE);
	...
	while (1) { //list loop
		...
		if (selection != oldselect) {
			...
			for (i=0; i<n; i++) { //display list
				...
				if (i == selection) {
					printXY(FileEntry[i].displayname, menu_x1+10 + FONT_WIDTH*2, i*16
